...
if(isset($_POST['edit-submit'])){
  ...
  if(empty($first) || empty($last) || empty($email) || empty($city)){
    header("Location: ../profile/profile.php?empty=error");
    exit();
  } else {
    ...
    if($resultRows > 0){
      header("Location: ../profile/profile.php?email=taken");
      exit();
    } else {
      ...
      header("Location: ../profile/profile.php?edit=success");
      exit();
    }
  }
} else {
  header("Location ../profile/profile.php?edit=error");
  exit();
}
