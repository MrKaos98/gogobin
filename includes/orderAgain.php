...
if(isset($_POST['submit'])){
  ...
  if(!mysqli_stmt_prepare($stmt, $getPrevOrder)){
    ...
    if($resultRows > 0){
      ...
      header("Location: ../homepage/foodbin.php?orderagain=progress");
      exit();
    } else {
      header("Location: ../homepage/foodbin.php?noresults=error");
      exit();
    }
  }
} else {
  header("Location: ../profile/profile.php?submit=error");
  exit();
}
